...
var sse1 = function () {
    ...
    return {
        buildMenu: function () {
            ...
            var url = document.location.href.toLowerCase().substring(0, lastIndex);
            
            k = -1;
            var nLength = -1;
            for (var i = 0; i < a.length; i++) {
            	var lastIndexA = a[i].href.lastIndexOf('/');
            	var urlA = a[i].href.toLowerCase().substring(0, lastIndexA);
                if (urlA == url && a[i].href.length > nLength) {
                    k = i;
